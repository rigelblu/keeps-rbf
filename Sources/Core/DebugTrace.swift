...
public enum DebugTrace {
  ...
  public static let path: String? = {
    ...
    return (v == "1" || v.lowercased() == "true") ? "/tmp/keeps-debug.log" : v
  }()
  ...
  public static func log(_ line: String) {
    guard let path, let data = (stamp() + " " + line + "\n").data(using: .utf8) else { return }
    if let h = FileHandle(forWritingAtPath: path) {
      defer { try? h.close() }
      h.seekToEndOfFile()
      h.write(data)
    } else {
      try? data.write(to: URL(fileURLWithPath: path))  // first write creates the file
    }
  }
