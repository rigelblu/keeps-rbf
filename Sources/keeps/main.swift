   ...
   private func notifyVerdict(_ r: Carry.CarryResult) {
     ...
     UNUserNotificationCenter.current().requestAuthorization(options: [.alert, .sound]) { [weak self] granted, err in
       ...
       guard granted else {
         ...
         // #keeps-20: UNUserNotifications completion handlers run on a framework background queue, and
         // the publish path writes AppKit state (status-item title, menu items). Hop to main first — the
         // same idiom this file already uses for the notification-tap handler.
         DispatchQueue.main.async { self?.refreshStandingConditions() }
         return
       }
       let center = UNUserNotificationCenter.current()
       if !offerStands {
