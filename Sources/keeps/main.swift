<<<<<<< conflict 1 of 1
%%%%%%% diff from: qskotswq a8965fc4 "docs: README, CHANGELOG, and why macOS window-restore is hard" (parents of rebased revision)
\\\\\\\        to: qskotswq 89a62c49 "docs: README, CHANGELOG, and why macOS window-restore is hard" (rebase destination)
...
-app.run()
+++++++ oryrwoto eb9d62c2 "feat: restore windows to their captured display/position/size on config change (silent AX, active-desktop slice) (#stay-3)" (rebased revision)
// keeps — the menu-bar host (#keeps-2). Modes:
//   (default)        menu-bar app: a status item + "Save Workspace Layout" + Quit
//   --capture-once   capture → store once, print a summary, exit   (Scenario A/E self-verify)
//   --print          capture → print JSON to stdout (no store), exit
