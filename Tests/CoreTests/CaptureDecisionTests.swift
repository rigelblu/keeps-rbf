...
// This is Scenario A made executable: the #keeps-6 regression (silently losing real windows) becomes
...
@Suite struct CaptureDecisionTests {
  let index: [Int: Capture.SpaceLoc] = [
    9: .init(displayUUID: "DISP-A", ordinal: 3, spaceUUID: "SPACE-9")
  ]
  ...
  private func candidate(
    ...
    Capture.Candidate(
      bundleId: bundleId, pid: 42, layer: layer, wid: wid,
  ...
  @Test func keepsRealPlacedWindowWithAttribution() throws {
    guard case .keep(let w) = Capture.decide(candidate(), spaceIndex: index) else {
      Issue.record("expected a real single-space window to be kept")
      return
    }
  ...
  @Test func allDesktopsWindowIsKeptButFlaggedSticky() throws {
    guard case .keep(let w) = Capture.decide(candidate(spaces: [9, 10, 11]), spaceIndex: index)
    else {
      Issue.record("expected an all-desktops window to be kept (flagged)")
      return
    }
  ...
  @Test func dropsDegenerateFrame() {
    #expect(
      Capture.decide(candidate(frame: WindowFrame(x: 0, y: 0, w: 0, h: 0)), spaceIndex: index)
  ...
  @Test func unknownSpaceKeepsWindowUnattributed() throws {  // on one space, but not in our index
    guard case .keep(let w) = Capture.decide(candidate(spaces: [999]), spaceIndex: index) else {
      Issue.record("a single-space window should still be kept even if the space isn't indexed")
      return
    }
